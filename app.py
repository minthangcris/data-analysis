...
# --- PHÂN TÍCH DỮ LIỆU ---
    ...
    sns.lineplot(data=sales_by_date, x="Date", y="Sales_Quantity", marker="o", ax=ax1)
    ...
    st.header("2️⃣ Tổng số lượng bán theo sản phẩm")
    product_sales = df.groupby("Product_ID")["Sales_Quantity"].sum().sort_values(ascending=False)
    fig2, ax2 = plt.subplots(figsize=(8, 4))
    sns.barplot(x=product_sales.index, y=product_sales.values, palette="viridis", ax=ax2)
    ax2.set_title("Total Sales by Product")
    ax2.set_xlabel("Product ID")
    ax2.set_ylabel("Sales Quantity")
    st.pyplot(fig2)

    st.header("3️⃣ Tỉ lệ kiểm tra chất lượng")
    quality_counts = df["Quality_Check_Pass"].value_counts().sort_index()
    fig3, ax3 = plt.subplots(figsize=(6, 4))
    sns.barplot(x=["Fail", "Pass"], y=quality_counts.values, palette="coolwarm", ax=ax3)
    ax3.set_title("Quality Check Pass/Fail Count")
    ax3.set_ylabel("Count")
    st.pyplot(fig3)

    st.header("4️⃣ Phân phối thời gian giao hàng")
    fig4, ax4 = plt.subplots(figsize=(6, 4))
    sns.histplot(df["Delivery_Time_Days"], bins=4, kde=True, ax=ax4)
    ax4.set_title("Delivery Time Distribution")
    ax4.set_xlabel("Delivery Days")
    st.pyplot(fig4)

    st.header("5️⃣ Mối liên hệ: Downtime và lỗi chất lượng")
    fig5, ax5 = plt.subplots(figsize=(6, 4))
    sns.scatterplot(data=df, x="Machine_Downtime_Hours", y="Quality_Issue_Count", hue="Machine_ID", ax=ax5)
    ax5.set_title("Downtime vs. Quality Issues")
